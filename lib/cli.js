...
function gitrefs() {
  spawn("git", ["show-ref", "--head", "--dereference"], { cwd: config.source }, function(err, out) {
    ...
    for(var i = 0; i < refs.length && !found; ++i) {
      var ref = refs[i];
      ...
      if(name === 'HEAD')
        HEAD = hash;
      if(!repoRef && /refs\/remotes\/[^\/]+\/HEAD/.test(name))
        repoRef = hash;
      if(hash.indexOf(repoRef) === 0 && hash === HEAD)
        found = true;
      if(name.indexOf(repoRef) >= 0 && hash === HEAD)
        found = true;
    }
...
function checkout() {
  ...
  spawn("git", ["checkout", repoRef], { cwd: config.source }, function(err) {
...
function validateSource() {
  ...
  child.execFile("git", ['remote','-v','show'], {
    cwd: config.source
...
function spawn() {
  ...
  proc.on('close', function (code) {
    callback(code === 0 ? null : "error", stdout, stderr);
  });
  return proc;
}
